...
export function exportWrite(funscriptPath, { mode = 'forge', out = null, blendSeams = false, finalSmooth = false, matchChapterVolume = false, stem = null, media = null, stimWav = false, stimMp3 = false, includeMedia = false, exclude = [], previews = null } = {}) {
  return call(
    'export_write',
    // `previews` is { funscript?, audio?, spectrogram? } of PNG data URLs
    // snapshotted from the player's lanes; Rust writes them to temp files and
    // passes the paths so cli.py packs the viewer's own pixels (not matplotlib).
    { funscriptPath, mode, out, blendSeams, finalSmooth, matchChapterVolume, stem, media, stimWav, stimMp3, includeMedia, exclude, previews },
    () => Promise.resolve(null),
  );
}
