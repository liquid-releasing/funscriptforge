...
const OPTION_DEFS = [
  ...
  { id: 'matchChapterVolume', label: 'Match volume across chapters', desc: 'A safety net for older work. E-stim channels used to be built one chapter at a time, each starting near silence, so every chapter change dropped in level; they are now built with overlap and share one volume ramp across the whole scene, so there is no longer a step to repair. Leave it on for channels made by an earlier version or joined elsewhere. Scene Builder and Scene Closer chapters keep their ramps.', estim: true },
