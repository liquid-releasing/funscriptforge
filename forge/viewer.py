...
def load_device_outputs(path: str, *, max_points: int = 2000) -> dict:
    """Scan ``<stem>.output/`` for device channel funscripts.

    ``path`` is the project's media or funscript path; the output dir is its
    sibling ``<stem>.output``. Returns
    ``{available, devices:[{name, channels:[{name, actions}]}], durationMs, screech}``.
    """
    p = Path(path)
    stem = p.stem
    output_dir = p.parent / f"{stem}.output"
    if not output_dir.is_dir():
        return {"available": False, "error": "no <stem>.output dir", "devices": []}
