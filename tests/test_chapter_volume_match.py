...
back. `match_chapter_seams` holds the plateau across those seams.
...
class TestSeamRepair(unittest.TestCase):
    ...
    def test_the_notch_at_an_internal_seam_is_filled(self):
        ...
        out = match_chapter_seams("volume", acts, windows)
    ...
    def test_an_upward_spike_at_the_seam_is_also_removed(self):
        # pulse_rise_time doesn't notch DOWN at a boundary — it spikes UP
        # (measured: a plateau of ~30 jumping to 70, then decaying over 10s).
        # An earlier version only ever raised values, which made that half of
        # the problem structurally unfixable.
        windows = [(0, 300_000, "balanced"), (300_000, 600_000, "balanced")]
        acts = ([{"at": t, "pos": 30} for t in range(0, 299_500, 500)]
                + [{"at": 300_000, "pos": 70}]
                + [{"at": t, "pos": 30} for t in range(300_500, 600_001, 500)])
        out = match_chapter_seams("pulse_rise_time", acts, windows)
        spike = next(a["pos"] for a in out if a["at"] == 300_000)
        self.assertLess(spike, 40, "upward spike survived the repair")

    def test_the_repaired_window_joins_its_surroundings(self):
        # Replacement must not introduce a NEW discontinuity at the window
        # edges — the line is anchored to the plateau on each side.
        windows, acts = _two_chapters()
        out = match_chapter_seams("volume", acts, windows)
        for a, b in zip(out, out[1:]):
            self.assertLess(abs(b["pos"] - a["pos"]), 25,
                            f"jump of {b['pos'] - a['pos']} at {b['at']}ms")
...
class TestGuards(unittest.TestCase):
    ...
    def test_phase_channels_pass_through(self):
        # alpha/beta encode WHERE the sensation sits, so a dip in them is real
        # motion rather than a seam artifact — repairing it would change the
        # sensation instead of the boundary.
        windows, acts = _two_chapters()
        for ch in ("alpha", "beta", "alpha-prostate", "beta-prostate"):
            self.assertEqual(match_chapter_seams(ch, acts, windows), acts)

    def test_every_continuous_parameter_channel_is_repaired(self):
        # User, 2026-08-16: "intensity, frequency, volume, pulse rise time,
        # volume-prostate ... none of these should be affected by a chapter
        # transition." An earlier version covered only the two volume channels.
        windows, acts = _two_chapters()
        for ch in SEAM_MATCHED_CHANNELS:
            out = match_chapter_seams(ch, acts, windows)
            self.assertGreater(
                min(a["pos"] for a in out if 295_000 <= a["at"] <= 305_000), 85,
                f"{ch} still notched at the seam",
            )

    def test_single_chapter_is_a_no_op(self):
        acts = _chapter(0, 300_000)
        self.assertEqual(match_chapter_seams("volume", acts, [(0, 300_000, "balanced")]), acts)

    def test_empty_actions(self):
        self.assertEqual(match_chapter_seams("volume", [], [(0, 1), (1, 2)]), [])

    def test_windows_with_missing_bounds_are_skipped(self):
        windows = [(0, 300_000, "balanced"), (None, None, "balanced")]
        _, acts = _two_chapters()
        self.assertEqual(match_chapter_seams("volume", acts, windows), acts)
