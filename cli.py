...
def cmd_export(args):
    ...
    try:
        ...
        manifest = {
            ...
            # Which OUTPUT pipeline built this. Absent = built before stamping
            # existed, which means the motion track was flattened by the old
            # blend_seams. See forge/pipeline_version.py.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # --- refresh (re-render outputs from the existing working state) ---
    p_ref = sub.add_parser(
        "refresh",
        help="Re-render a project's (or a whole library's) outputs after an "
             "engine fix, without re-authoring anything",
    )
    p_ref.add_argument(
        "path",
        help="A .funscript, or a folder to walk for projects (anything with a "
             "`.<stem>.forge/` working dir).",
    )
    p_ref.add_argument(
        "--check", action="store_true",
        help="Report status as JSON and change nothing. This is what a UI "
             "prompt reads to decide whether to offer a refresh.",
    )
    p_ref.add_argument(
        "--all", action="store_true",
        help="Include projects that are already current (default: only stale).",
    )
    p_ref.add_argument(
        "--export-only", action="store_true",
        help="Skip re-generating device stations and only re-export. Correct "
             "and much faster when only the export stage changed.",
    )
...
def cmd_polish_write(args):
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    from forge.pipeline_version import OUTPUT_PIPELINE_VERSION
    # The stations in `passes` were generated by this pipeline version. A
    # mismatch on read means the stamped channel files are out of date even
    # though the stamps themselves are still valid.
    doc = {"version": 1, "schema": "polish/v1",
           "pipeline_version": OUTPUT_PIPELINE_VERSION, "passes": passes}
    path.write_text(yaml.safe_dump(doc, sort_keys=False, allow_unicode=True),
                    encoding="utf-8")
...
# ---------------------------------------------------------------------------
# refresh -- re-render a project's outputs from its existing working state
#
# "I just want to update the existing forge files with the new dynamics, not
# start over." Everything needed is already in `.<stem>.forge/`: the authored
# work funscript, the chapter + character sidecars, and polish.yml recording
# which stations were stamped with which knobs. So a refresh is a re-render,
# not a re-authoring.
#
# WARNING: it rebuilds at the project's CURRENT state, not the state that
# produced the old bundle. If the work funscript changed since, that change
# comes along. Almost always what you want -- but it is a rebuild, not a replay.
# ---------------------------------------------------------------------------

def _forge_dir_for(funscript: str):
    from videoflow.sidecar import forge_dir
    return forge_dir(funscript)


def _work_funscript_for(funscript: str) -> str:
    """The edited motion to render from, or the original when nothing is edited."""
    stem = Path(funscript).stem
    work = _forge_dir_for(funscript) / f"{stem}.work.funscript"
    return str(work) if work.exists() else funscript


def _bundle_for(funscript: str):
    """The default .forge bundle path for a project (may not exist)."""
    stem = Path(funscript).stem
    return Path(funscript).parent / f"{stem}.forge"


def _read_bundle_manifest(bundle):
    """The manifest from a .forge zip or an unzipped bundle folder, or None."""
    try:
        if bundle.is_dir():
            mp = bundle / "manifest.ffmeta"
            return json.loads(mp.read_text(encoding="utf-8")) if mp.exists() else None
        if bundle.exists():
            import zipfile
            with zipfile.ZipFile(bundle) as z:
                return json.loads(z.read("manifest.ffmeta"))
...
def project_status(funscript: str) -> dict:
    ...
    Shaped for a UI prompt as much as for the CLI: `state` decides whether to
    ...
    from forge.pipeline_version import (
        OUTPUT_PIPELINE_VERSION, is_stale, changes_since,
    )
    ...
    passes, polish_version = {}, None
    ppath = _polish_path(funscript)
    if ppath.exists():
        try:
            doc = _yaml.safe_load(ppath.read_text(encoding="utf-8")) or {}
            passes = doc.get("passes") or {}
            polish_version = doc.get("pipeline_version")
        except (OSError, _yaml.YAMLError):
            pass
    accepted = sorted(k for k, v in passes.items()
                      if isinstance(v, dict) and v.get("accepted"))

    available = sorted(_p.STATIONS.keys())
    missing = [s for s in available if s not in accepted]

    reasons = []
    if manifest is None:
        state = "never-exported"
        reasons.append("No bundle has been exported for this project yet.")
    elif is_stale(stamped):
        state = "stale"
        reasons.extend(changes_since(stamped))
    elif accepted and is_stale(polish_version):
        state = "stale"
        reasons.append(
            "The device channels were generated by an older pipeline than the "
            "bundle; re-stamping them picks up the newer generation.")
    else:
        state = "current"
    ...
    return {
        ...
        "stem": stem,
        "bundle": str(bundle) if manifest is not None else None,
        "state": state,
        "reasons": reasons,
        "pipeline_version": stamped,
        "polish_pipeline_version": polish_version,
        "current_pipeline_version": OUTPUT_PIPELINE_VERSION,
        "stations_stamped": accepted,
...
def _projects_under(root: str) -> list:
    """Every funscript under `root` that has a `.<stem>.forge/` working dir.

    The working dir is the marker: a bare funscript nobody has opened is not a
    project and has nothing to refresh. Our own outputs are skipped so a
    library scan does not try to refresh work copies or bundle contents.
    """
    out = []
    rp = Path(root)
    if rp.is_file():
        return [str(rp)]
    for fs in sorted(rp.rglob("*.funscript")):
        if fs.name.endswith(".work.funscript"):
            continue
        if any(part.endswith(".forge") or part.endswith(".output")
               for part in fs.parts[:-1]):
            continue
        if _forge_dir_for(str(fs)).is_dir():
            out.append(str(fs))
    return out


def _export_opts_from_manifest(manifest: dict, src: str) -> list:
    """Re-derive the export flags that produced a bundle, from the bundle.

    A refresh that silently drops content is worse than no refresh: the first
    run of this command rebuilt a project correctly but shipped a 3.6 MB
    bundle where the original was 87 MB, having quietly lost audio/stim.mp3,
    audio/stim-prostate.mp3 and every media-derived thumbnail. Those come from
    OPT-IN flags (--stim-mp3, --media) that nothing on disk recorded.

    The manifest's artifact list is the record: what a bundle contains says
    which flags built it. Inferring from artifacts rather than adding a new
    "export options" field also fixes bundles that predate any such field --
    which is every bundle that needs refreshing.
    """
    paths = {a.get("path", "") for a in (manifest.get("artifacts") or [])
             if isinstance(a, dict)}
    argv = []
    if any(p.endswith(".mp3") and p.startswith("audio/stim") for p in paths):
        argv.append("--stim-mp3")
    if any(p.endswith(".wav") and p.startswith("audio/stim") for p in paths):
        argv.append("--stim-wav")
    if not any(p.startswith("audio/beat") for p in paths):
        argv.append("--no-beat-mp3")
    media = manifest.get("media") or {}
    if media.get("bundled"):
        argv.append("--include-media")
    # Hero + per-chapter frame thumbnails need the media file. The manifest
    # records its basename (bundles are lean by default and relink on import),
    # so look for it beside the source.
    wants_frames = any(p.startswith("thumbnails/hero")
                       or p.startswith("thumbnails/chapter_") for p in paths)
    if wants_frames and media.get("filename"):
        cand = Path(src).parent / media["filename"]
        if cand.exists():
            argv += ["--media", str(cand)]
    return argv
...
def cmd_refresh(args):
    ...
    for st in statuses:
        src = st["funscript"]
        work = st["work_funscript"]
        _emit_progress(f"Refresh - {st['stem']}")
        parser = build_parser()

        # Re-generate each stamped station from the current work funscript so
        # generation-stage fixes reach the channel files. --export-only skips
        # this: it is the cheap path when only the export stage changed.
        if not args.export_only:
            passes = {}
            ppath = _polish_path(src)
            if ppath.exists():
                try:
                    passes = (_yaml.safe_load(ppath.read_text(encoding="utf-8"))
                              or {}).get("passes") or {}
                except (OSError, _yaml.YAMLError):
                    passes = {}
            for sid in st["stations_stamped"]:
                knobs = (passes.get(sid) or {}).get("knobs") or {}
                argv = ["polish-apply", src, "--effective", work, "--station", sid]
                if knobs:
                    argv += ["--params-json", json.dumps(knobs)]
                # Each command prints its own JSON; swallow it so refresh emits
        ...
        inherited = _export_opts_from_manifest(prev, src) if prev else []
        argv += inherited
        if args.out:
            argv += ["--out", args.out]
    ...
    print(json.dumps({"refreshed": len(done), "projects": done}, indent=2))
