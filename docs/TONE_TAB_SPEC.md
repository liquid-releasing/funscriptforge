...
Every export writes a `DISCLAIMER.txt` into the output folder alongside the
generated files. The disclaimer travels with the content.
...
```
These files were generated by FunscriptForge.

This content is intended for use by informed adults with compatible devices
operated according to the manufacturer's instructions.

FunscriptForge generates output within the published operating parameters of
supported devices. It does not guarantee a safe or suitable experience.
Physical devices involve inherent risk. You are responsible for your own
health, safety, and the condition of your equipment.

Use at your own risk.
```

The disclaimer is plain text, always overwritten on export, not user-editable.
It is not shown in the UI — it is part of the output package.
...
then applies the device limits pass before writing.

```
.forge (global=Tease, phrase_tones={3: Climax, 7: Tender})
    │
    ├── main funscript recipe  →  [limits pass]  →  my-scene.funscript
    ├── estim alpha recipe     →  [limits pass]  →  estim/alpha.funscript
    ├── estim beta recipe      →  [limits pass]  →  estim/beta.funscript
    ├── estim pulse_frequency  →  [limits pass]  →  estim/pulse_frequency.funscript
    ├── estim pulse_width      →  [limits pass]  →  estim/pulse_width.funscript
    ├── estim pulse_rise       →  [limits pass]  →  estim/pulse_rise.funscript
    ├── estim E1–E4, prostate  →  [limits pass]  →  estim/E1.funscript … prostate.funscript
    └── Handy funscript        →  [limits pass]  →  handy/my-scene.funscript
```
